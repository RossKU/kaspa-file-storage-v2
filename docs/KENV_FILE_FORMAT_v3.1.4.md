# KENV File Format Specification v3.1.4 - 統合最適化版

## 概要
KENVファイルシステムは、Kaspa P2P File Storageの作業環境を管理する軽量ストレージシステムです。v3.1.4では、v3.1.3の全機能に加え、ディレクトリタイプの明確な仕様定義とsaltカラムの追加を実現しました。
...
### v3.1.4での追加変更
1. **固定カラムを46個に修正**
   - saltカラムを46番目に追加（v3.1.3で記載漏れ）
2. **ディレクトリタイプの仕様明確化**
   - kaspa-directoryタイプの可変カラム構造を定義
   - エントリー形式の標準化（7カラム/エントリー）
3. **ドキュメント整備**
   - 固定カラムと可変カラムの明確な区分
   - ディレクトリエントリーの具体例追加
...
|---|---------|------|------|
| 1 | id | ✓ | ローカル一意識別子 |
| 2 | type | ✓ | kaspa-file/kaspa-directory |
| 3 | version | | フォーマットバージョン |
| 6 | name | ✓ | ファイル/ディレクトリ名 |
| 8 | metaTxId | | オンチェーンモード時必須 |
| 9 | metaTxBlockId | | オンチェーンモード時必須 |
| 14 | source | ✓ | upload/manual/download |
| 15 | status | ✓ | pending/verifying/verified/failed |
| 30 | totalFiles | | ディレクトリ内の総ファイル数 |
| 31 | fileCount | | 直下のファイル数 |
| 32 | directoryCount | | 直下のディレクトリ数 |
| 41 | passwordIncluded | | パスワード含有フラグ |
| 42 | password | | 暗号化パスワード（推奨保管） |
| 43 | externalRef | | 大容量ファイル外部参照 |
| 46 | salt | | 暗号化ソルト（Base64） |

### 固定カラム数
```javascript
const FIXED_COLUMN_COUNT = 46;  // v3.1.4で46カラム固定（saltを含む）
```
...
##### 最小構成ディレクトリ（空フォルダ）
```csv
dir_001,kaspa-directory,3.4.2,2025-08-03T10:00:00Z,testnet-10,EmptyFolder,,,,,,,,manual,pending,0,,,,,true,,,false,false,,0,0,,,0,0,0,0,,0,0,,,0,true,mypass,,work,空フォルダ,,YmFzZTY0c2FsdA==,entries,0,0
```
...
##### 複雑な階層構造（10ファイル、3ディレクトリ）
```csv
dir_003,kaspa-directory,3.4.2,2025-08-03T10:00:00Z,testnet-10,ComplexProject,,tx123,blk456,,2025-08-03T10:01:00Z,2025-08-03T10:01:30Z,kaspa:qq...,upload,verified,5242880,,,,,true,,,false,false,Qm123...,0,0,,,13,10,3,0,,0.00021,1234,,,0,true,secure456,,project,本番プロジェクト,,YmFzZTY0c2FsdA==,entries,10,3,f,README.md,,102400,aaa111,bbb222,,f,package.json,,10240,ccc333,ddd444,,f,index.js,src,20480,eee555,fff666,,f,utils.js,src/lib,15360,ggg777,hhh888,,f,test.js,test,30720,iii999,jjj000,,f,logo.png,assets/images,409600,kkk111,lll222,,f,style.css,assets/css,51200,mmm333,nnn444,,f,data.json,data,1024000,ooo555,ppp666,datapass,f,report.pdf,docs,3072000,qqq777,rrr888,docpass,f,backup.zip,backup,512000,sss999,ttt000,backuppass,d,src,,0,,,,d,assets,,0,,,,d,docs,,0,uuu111,vvv222,docpass
```
...
### パース実装
```javascript
const FIXED_COLUMN_COUNT = 46;  // v3.1.4で46カラム固定
...
function parseKENVEntry(line) {
    ...
    const entry = {
        ...
        salt: columns[45] || null  // 46番目のカラム
    };
    
    // 可変カラム処理
    if (columns.length > FIXED_COLUMN_COUNT) {
        if (entry.type === 'kaspa-directory' && columns[46] === 'entries') {
            // ディレクトリエントリーのパース
            const fileCount = parseInt(columns[47]) || 0;
            const dirCount = parseInt(columns[48]) || 0;
            const entries = [];
...
### CSVライン生成
```javascript
function generateDirectoryCSVLine(entry) {
    // 固定カラム（46個）
    const fixedColumns = new Array(46).fill('');
    
    // 必須フィールド
    fixedColumns[0] = entry.id;
    fixedColumns[1] = entry.type;
    fixedColumns[2] = entry.version || '';
    fixedColumns[5] = entry.name;
    fixedColumns[13] = entry.source;
    fixedColumns[14] = entry.status;
    // ... 他のフィールド
    fixedColumns[30] = entry.totalFiles || '';
    fixedColumns[31] = entry.fileCount || '';
    fixedColumns[32] = entry.directoryCount || '';
    fixedColumns[41] = entry.password || '';
    fixedColumns[42] = entry.externalRef || '';
    fixedColumns[45] = entry.salt || '';  // 46番目
    
    // 可変カラム
    const variableColumns = [];
    
    if (entry.type === 'kaspa-directory' && entry.entries) {
        variableColumns.push('entries');
        variableColumns.push(entry.entries.filter(e => e.type === 'file').length);
        variableColumns.push(entry.entries.filter(e => e.type === 'directory').length);
    ...
    return [...fixedColumns, ...variableColumns].map(escapeCSVValue).join(',');
}
```
...
async function verifyAndComplete(entry) {
    ...
    try {
        if (entry.type === 'kaspa-directory') {
            if (entry.metaTxId) {
                // オンチェーンディレクトリ
                const dirData = await fetchDirectoryMetadata(
                    entry.metaTxId,
                    entry.metaTxBlockId,
                    entry.password
                );
                ...
                Object.assign(entry, {
                    fileCount: dirData.directory.fileCount,
                    directoryCount: dirData.directory.directoryCount,
                    totalSize: dirData.directory.totalSize,
                ...
                // .kaspaファイルからディレクトリ
                const dirData = await loadLocalKaspaFile(entry.externalRef);
...
| エントリー数 | 推定カラム数 | 推定サイズ |
|-------------|-------------|------------|
| 10 | 46 + 3 + (7×10) = 119 | 約1KB |
| 100 | 46 + 3 + (7×100) = 749 | 約8KB |
| 1000 | 46 + 3 + (7×1000) = 7049 | 約80KB |

通常のファイル（1000チャンク）の約4000カラムと比較して、ディレクトリは効率的です。
...
// ディレクトリ検索
function searchDirectories(query) {
  const dirLines = kindexData.filterIndexes.type.directory;
  return dirLines.filter(line => {
    const entry = readCSVLine(line);
    return entry.name.toLowerCase().includes(query.toLowerCase());
  });
}
...
# スキーマ更新のみ（データ互換性あり）
# saltカラムを空で追加
sed -i 's/$/,/' workspace.kentry  # 各行末にカンマ追加

# KENVバージョン更新
jq '.version = "3.1.4"' workspace.kenv > workspace.kenv.tmp
mv workspace.kenv.tmp workspace.kenv
```
...
- **段階的検証** - バックグラウンドで情報補完
- **ディレクトリタイプ対応** - kaspa-directoryの完全サポート

### 技術面
- **専用拡張子の採用** - .kenv/.kindex/.kentry
- **.kindex最適化** - 10万件で約3MB（従来比85%削減）
- **46カラムCSV** - saltカラムを含む完全仕様
- **マジックナンバー外部化** - 設定による調整可能
- **効率的な可変カラム** - ディレクトリは7カラム/エントリー
...
- **ストリーミング対応** - 大規模CSV処理
- **オフライン対応** - ローカル完結モード
- **階層構造の保持** - pathによる完全な階層表現

これにより、大規模ファイルリストでも快適な操作性と、多様な利用シナリオへの対応を両立しています。特にディレクトリタイプの追加により、複雑な階層構造も効率的に管理できるようになりました。